...
import { HelpCircle, Loader2, ShieldQuestion, X } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { getDeletionQuestion } from "@/lib/delete-resource";

type SecurityAnswerDialogProps = {
  open: boolean;
  title: string;
  description?: string;
  confirmLabel: string;
...
export default function SecurityAnswerDialog({
  open,
  title,
  description,
  confirmLabel,
    ...
    void (async () => {
      try {
        ...
        if (!security.serverConfigured) {
          setQuestionState({ status: "error", message: "Security verification is not available right now." });
        ...
        >
          ...
          {questionState.status === "ready" && (
            <form onSubmit={handleSubmit}>
              ...
              <label htmlFor="security-verify-answer" className="mb-2 block text-sm font-semibold">Your answer</label>
              <Input
              ...
              <div className="mt-5 grid grid-cols-2 gap-2">
                ...
                <Button type="submit" disabled={submitting || !answer.trim()}>
                  {submitting ? <Loader2 className="animate-spin" /> : <ShieldQuestion />}
